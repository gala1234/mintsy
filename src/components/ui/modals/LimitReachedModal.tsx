...
const LimitReachedModal: React.FC<LimitReachedModalProps> = ({
  ...
  return (
    <Backdrop isOpen={isOpen} className="bg-background/90" onClick={onClose}>
      <div
        className={cn(
          "bg-[#FDFBF7] rounded-xl shadow-lg max-w-md w-full mx-auto overflow-hidden animate-fadeIn modal-content modalContainer relative"
        )}
      ...
      >
        ...
        <div className="flex flex-col items-center px-8 pt-10 pb-4">
          ...
          {/* Message */}
          <p className="text-text-muted mb-8 text-center">{content.message}</p>

          {/* Buttons */}
          <div className="space-y-4 w-full mb-6">
            <LinkButton href={upgradeUrl} variant="primary" className="w-full">
              Upgrade to Pro
            </LinkButton>

            <LinkButton href={buyOneUrl} variant="secondary" className="w-full">
        ...
        <div className="pt-4 pb-10 border-t border-mint/20 w-full">
          <p className="text-sm text-text-muted mb-4">Share your creation on</p>
